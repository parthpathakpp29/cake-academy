        ...
        try {
            ...
            if (req.files.video) {
                for (let i = 0; i < req.files.video.length; i++) {
                    ...
                    const lectureTitle = lectureTitles[i] || `Lecture ${i + 1}`;
                    const videoResult = await uploadVideo(videoFile, "temp", i + 1);
                    lectureResults.push({
                        title: lectureTitle,
            ...
            if (req.files.video) {
                req.files.video.forEach(video => fs.unlinkSync(video.path));
            }
